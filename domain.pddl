...
(define (domain coffee_robot)
    ...
    (:requirements :strips :fluents :durative-actions :timed-initial-literals :typing :conditional-effect :negative-preconditions :duration-inequalities :equality :time ::timed-effects :adl)
    ...
    (:predicates
        ...
        (uf ?r -robot) ; moving up fast flag
        (lf ?r -robot) ; moving left fast flag
        (rf ?r -robot) ; moving right fast flag
        (df ?r -robot) ; moving down fast flag
        (ulf ?r -robot) ; moving up left fast flag
        (urf ?r -robot ) ; moving up right fast flag
        (dlf ?r -robot) ; moving down left fast flag
        (drf ?r -robot) ; moving down right fast flag

        (us ?r -robot) ; moving up slow flag
        (ls ?r -robot) ; moving left slow flag
        (rs ?r -robot) ; moving right slow flag
        (ds ?r -robot) ; moving down slow flag
        (uls ?r -robot) ; moving up left slow flag
        (urs ?r -robot) ; moving up right slow flag
        (dls ?r -robot) ; moving down left slow flag
        (drs ?r -robot) ; moving down right slow flag 
    ...
    (:action Put_down_cold_drink
        ...
        :precondition (and (waiter ?r) (= (xr ?r) (xt ?t)) (= (yr ?r) (yt ?t)) (drink_holded ?d ?r) (not (busy ?r)) (cold_drink ?d))
        :effect (and (decrease (holding ?r) 1) (not (drink_holded ?d ?r)) (drink_on_table ?d ?t) (served_cold_drink ?d ?t))
    )
    (:action Put_down_hot_drink
        :parameters (?r -robot ?t -table ?d -drink)
        :precondition (and (waiter ?r) (= (xr ?r) (xt ?t)) (= (yr ?r) (yt ?t)) (drink_holded ?d ?r) (not (busy ?r)) (hot_drink ?d))
